...
export default function DemoControls() {
  ...
  // Keep a ref so the interval callback always has the latest selectedSystem
  const selectedSystemRef = useRef(selectedSystem);
  selectedSystemRef.current = selectedSystem;

  const fetchSystems = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/demo/systems`);
      if (!res.ok) {
      ...
      setSystems(list);
      setLoadError(null);
      if (list.length > 0 && !selectedSystemRef.current) {
        setSelectedSystem(list[0].system_id);
      }
    } catch (e) {
      setLoadError("Cannot reach backend — is the server running?");
    }
  };
  ...
  const handleSetState = async () => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/demo/set-state/${selectedSystem}`,
        {
          method: "POST",
      ...
      if (res.ok) {
        ...
        setConfirmation(`State set to ${result.state} at cycle ${result.cycle}`);
        narration.playNarration(selectedState);
        setTimeout(() => setConfirmation(null), 4000);
        await fetchSystems();
      } else {
        const body = await res.text();
        setConfirmation(`Error ${res.status}: ${body}`);
        setTimeout(() => setConfirmation(null), 4000);
      }
    } catch (e) {
      setConfirmation(`Network error: ${e.message}`);
      setTimeout(() => setConfirmation(null), 4000);
    } finally {
      setLoading(false);
    }
  ...
  const isError = confirmation && confirmation.startsWith("Error");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-zinc-900 bg-[#0A0A0A] p-6">
        <h2 className="font-mono text-sm font-bold text-zinc-100 mb-4 tracking-wider uppercase">
          Manual Controls
        </h2>
        <p className="font-mono text-xs text-zinc-500 mb-6">
          Manually drive system state for demonstrations
        </p>

        {/* Backend error */}
        {loadError && (
          <div className="mb-4 p-3 bg-red-950 border border-red-900">
        ...
        <div className="mb-6">
          ...
          >
            {systems.length === 0 ? (
              <option value="">{loadError ? "No systems available" : "Loading systems…"}</option>
            ) : (
              systems.map((sys) => (
          ...
          {currentSystem && (
            <div className="mt-2 font-mono text-[10px] text-zinc-500">
              Current:{" "}
              <span style={{ color: STATE_COLORS[currentSystem.current_state] }}>
                {currentSystem.current_state}
              </span>{" "}
              · Cycle {currentSystem.cycle}
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {STATES.map((state) => (
              <button
                ...
                style={{
                  backgroundColor:
                    selectedState === state ? STATE_COLORS[state] + "40" : "transparent",
                  borderColor:
                    selectedState === state ? STATE_COLORS[state] : "#3f3f46",
        ...
        {confirmation && (
          <div
            className={`mb-4 p-3 flex items-center gap-2 border ${
              isError
                ? "bg-red-950 border-red-900"
                : "bg-emerald-950 border-emerald-900"
            }`}
          >
            <Check className={`w-4 h-4 flex-shrink-0 ${isError ? "text-red-400" : "text-emerald-500"}`} />
            <p className={`font-mono text-xs ${isError ? "text-red-400" : "text-emerald-500"}`}>
              {confirmation}
            </p>
          </div>
        )}
      ...
      <div className="border border-zinc-900 bg-[#0A0A0A] p-4">
        <p className="font-mono text-[10px] text-zinc-500">
          Select a system, choose a target state, adjust metrics if needed, then click SET STATE.
          Changes persist for 60 seconds then simulation resumes.
        </p>
      </div>
    </div>
  );
